<template>
  <div class="bg-white rounded-lg border border-gray-200 p-6">
    ...
    <div class="space-y-3">
      <div
        v-for="(plan, index) in validPlans"
        :key="plan.id"
        class="relative"
      >
        <!-- Member Label -->
        <div class="text-sm font-medium text-gray-700 mb-2">
          {{ getMemberName(plan.memberId) }} - {{ getPositionName(plan.position) }} ({{ plan.allocationRate }})
        </div>
      ...
      <!-- Empty State -->
      <div v-if="validPlans.length === 0" class="text-center py-8 text-gray-500">
        <svg class="mx-auto h-12 w-12 text-gray-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 01-2 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
        </svg>
...
const validPlans = computed(() => {
  return props.plans.filter(p => p.startDate && p.endDate && p.memberId && p.position)
})
...
const getTimelineStyle = (plan) => {
  ...
  const duration = Math.min(endDate.getTime(), timelineEnd.getTime()) - Math.max(startDate.getTime(), timelineStart.getTime())
